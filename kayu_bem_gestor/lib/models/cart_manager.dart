...
import 'package:kayu_bem_gestor/models/cepaberto_address.dart';
import 'package:kayu_bem_gestor/services/cepaberto_service.dart';
...
class CartManager extends ChangeNotifier {
  ...
  Future<void> _carregarItensCarrinho() async {
    ...
    items = qs.docs.map((d) => CartProduct.fromDocument(d)..addListener(_atualizarItem)).toList();
  }

  Future<void> _carregarEndereco() async {
    if (usuario!.endereco != null && await calcularEntrega(usuario!.endereco!.lat!, usuario!.endereco!.long!)) {
  ...
  void adicionarProdutoCarrinho(Product produto) {
    try {
      final p = items.firstWhere((p) => p.verificarProdutoJaAdicionado(produto));
      p.incrementar();
    } catch (e) {
      final produtoCarrinho = CartProduct.fromProduct(produto);
      produtoCarrinho.addListener(_atualizarItem);
      items.add(produtoCarrinho);
      usuario!.cartRef.add(produtoCarrinho.converterCarrinhoMap()).then((doc) => produtoCarrinho.id = doc.id);
      _atualizarItem();
    }
    notifyListeners();
  }
  ...
  Future<void> getEnderecoCepAberto(String cep) async {
    ...
    try {
      final cepAbertoEndereco = await cepAbertoService.encontrarEnderecoPorCep(cep);
  ...
  Future<void> getEndereco(Endereco endereco) async {
    ...
    if (await calcularEntrega(this.endereco!.lat!, this.endereco!.long!)) {
      ...
      loading = false;
      return Future.error('Infelizmente ainda não estamos entregando para sua localidade.');
    }
  }
  ...
  Future<bool> calcularEntrega(double lat, double long) async {
    ...
    // a API do google
    double distancia = await Geolocator.distanceBetween(latOrigem, longOrigem, lat, long);
